...
import com.myapp.member.domain.traffic.service.TrafficStatsService;
...
public class AdminController {
    ...
    public AdminTrafficResponseDTO adminTrafficApi() {
        return trafficStatsService.readAdminTraffic();
    }

    @PatchMapping("/admin/users/{username}/lock")
    public ResponseEntity<Map<String, Long>> lockUserApi(
            @PathVariable String username
    ) {
        Long id = userService.lockUserByAdmin(username);
        return ResponseEntity.ok(
                Collections.singletonMap("userEntityId", id)
        );
    }

    @PatchMapping("/admin/users/{username}/unlock")
    public ResponseEntity<Map<String, Long>> unlockUserApi(
            @PathVariable String username
    ) {
